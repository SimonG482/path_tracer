...
public:
    ...
    const double delta = sqr(dot(ray.u, ray.O - C)) - ((ray.O - C).norm2() - R*R);
    if (delta < 0) {
      return false;
    } else if (delta == 0) {
      t = dot(ray.u, C - ray.O);
      ...
      const double t1 = dot(ray.u, C - ray.O) - std::sqrt(delta);
      const double t2 = dot(ray.u, C - ray.O) + std::sqrt(delta);
...
public:
    ...
    for (size_t i = 0; i < objects.size(); ++i) {
      ...
      if (objects[i]->intersect(ray, newP, newt, newN)) {
        if (newt < t) {
          P = newP;
          t = newt;
          N = newN;
	...
	Vector getColor(const Ray& ray, int recursion_depth) {
        ...
        //
        //
      const double maxposdot = dot(N, (light_position - P)/(light_position-P).norm());
      if (maxposdot > 0) {
